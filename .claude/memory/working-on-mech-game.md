...
**How to apply**:
- Put pure logic in `src/data/*` — damage, kill rules, loadout validation, hexgrid, save
  round-trip — so it can be reasoned about and exercised by hand. (This bullet used to say
  "and unit-test it with Vitest"; that suite is gone, see below.)
...
- **The owner plays the live game himself — don't drive the browser preview to visualize
  ...
  A green `npm run build` is sufficient evidence to report and merge on for changes he'll
  personally exercise. Reserve driving the Claude Preview browser
  ...
  himself.
- There is no automated test suite and no smoke harness — the Vitest suite was deleted
  2026-07-26 and `npm run smoke` 2026-07-31 (#329), both at his explicit direction. Don't
  add either back. The `scripts/audit-*.mjs` / `scripts/profile-*.mjs` one-offs still drive
  headless Playwright, which needs a one-time `npx playwright install chromium` (a heavy
  install — flag before running, per the global working-style in `~/.claude/CLAUDE.md`).
