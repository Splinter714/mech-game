...
describe('sfxOverridePanelState (#177 generalized id/stage panel display state)', () => {
  ...
  // #479: the GAIT cues (footstep/legLift) are now FILE bakes (multi-variant .m4a pools), like
  // mechDestroyed — the last synth bake (and the `isSynth` labeling that distinguished it) was
  // retired. A baked variant row reads as the "(baked) shipped sound" file wording.
  it('labels a multi-variant FILE bake (gait cue) as a "(baked) shipped sound" row', () => {
    for (let i = 0; i < 6; i++) {
      _setBakedBufferForTest('legLift', 'play', { duration: 2.5, numberOfChannels: 2, sampleRate: 48000 }, i);
    }
    expect(getVariantSlotCount('legLift', 'play')).toBe(6);
    const rows = getVariantRowStates('legLift', 'play');
    expect(rows).toHaveLength(6);
    for (const row of rows) {
      expect(row.source).toBe('baked');
      expect(row.statusText).toMatch(/baked/i);
    }
  });
