...
export default class GarageScene extends Phaser.Scene {
  ...
  _buildColumn(i) {
    ...
    // #505 sixth rework (playtest): a thin top-edge border on the panel — a visible dividing line
    // between it and the scrollable weapon catalog above — plus an invisible click-blocker sized
    // to the panel's FULL extent (`gl.panel`, flush with the column's own bottom padding now that
    // the old footer dead-space is gone). WeaponCardList masks its scrolled-out cards visually
    // (see weaponCardList.js's geometry mask) but a mask doesn't stop them from still being
    // INPUT-live wherever they're positioned in world space — without this blocker, a card
    // scrolled far enough down sits invisibly underneath the panel and still catches the click.
    // Added FIRST (before any tile), so within `col.layer` every tile still renders — and hit-
    // tests — on TOP of it; only the gaps between tiles (and the halo/dead space around them) fall
    // through to the blocker instead of whatever's behind it.
