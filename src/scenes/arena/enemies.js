...
// #574: AI tuning constants + pure helper functions moved to enemyAiTuning.js — imported back
// here since the mixin methods below use them throughout. See that file's header for context.
import {
  blockedForEnemy, SQRT3,
  BRAWLER_OPT, SNIPER_OPT, STANDOFF_MIN, STANDOFF_MAX, STANDOFF_FRAC, DEFAULT_OPT,
  TOO_CLOSE_FRAC, TOO_FAR_FRAC, DECIDE_MIN, DECIDE_MAX,
  FLANK_ANGLE_MIN, FLANK_ANGLE_MAX, FLANK_REACH,
  COVER_SEARCH_STEP, COVER_SEARCH_RING, COVER_HEALTH_TRIGGER, COVER_DAMAGE_WINDOW, PEEK_DIST,
  ARTY_RECAMP_MIN, ARTY_RECAMP_MAX,
  OFFSCREEN_MARGIN, SPAWN_WORLD_INSET,
  MOVE_SPEED_FRAC, ENEMY_MECH_ACCEL_FRAC,
  ...
  PLAYER_FLEE_DOT, PLAYER_VULN_HEALTH, TRACKED_DOT, TRACKED_BREAK_CHANCE,
  vehicleTextureKey, VEHICLE_SCALE_MULT, vehicleScale,
  rand, clamp, detent,
  meanOpt, roleFor, isIndirectWeapon, isAllIndirect, lethalHealth,
...
export const EnemiesMixin = {
  ...
  _spawnKind(x, y, typeId, statKind) {
    ...
    const e = {
      key, mech: body, view, x, y, vx: 0, vy: 0, angle, turret: angle,
  ...
  _resetEnemies() {
    for (const e of this.enemies) {
      ...
      if (e.kind === 'mech') {
        ...
        // #472: nothing to re-skin — a vehicle's textures are static now that the armor visual
        // is gone, so a repair changes no art.
      }
    }
  },
  ...
  _updateEnemies(dt, delta) {
    ...
    // #460: `delta` drives the staggered player→enemy hard-cover sight refresh that rule 3 of the
    // fog now consults (arena/visibility.js `_refreshPlayerSight`) — same cadence/stagger as the
    // enemy-side `_cachedLosToPlayer`.
    this._syncEnemyFogVisibility?.(delta);
  ...
  _updateVehicle(e, dt, delta) {
    ...
    if (e.kindDef.turretFrames) {
      e.view.turret.setTexture(`${e.key}_turret_${e.turretFrame ?? 0}`);
    }
  ...
  _fireEnemyShots(w, plan, mx, my, fireAngle, e) {
    // #423: one enemy trigger pull (whatever this plan emits). The returned shot id is threaded to
