...
// #427 removed the open-gate aim PIP (`openGateOf`/`GATE_PIP_HIT_RADIUS`, superseding #412): an
// open gate is now TWO leaves that only part ~70%, so it is solid to fire everywhere along its span
// (wallEdges.js `blocksShot`). Every round and beam already detonates on it like any other span, so
// no separate "is this my target" pip is needed to STOP a shot the solidity rule would have passed.
// It stays fully lockable as a standing span, exactly as #318 makes every span lockable.
